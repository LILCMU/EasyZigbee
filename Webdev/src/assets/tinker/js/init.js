...
var normalWS = function(){
	if (connectedMode != CONNECT_MODE.WS) return;
...
var startCheckWSNotResponse = function() {
	setInterval(function () {
		if (new Date() - window.lastWSHandle > 8000 && window.ws!=undefined){
			window.ws.close();
		}
	}, 8000);
}
...
var setConnectMode = function(mode) {
		connectedMode = mode
}

var setNextConnectMode = function() {
		connectedMode++;
		connectedMode %= CONNECT_MODE.MAX+1;
		if (connectedMode==0) {
			setNextConnectMode();
		}
}

var setIconStatus = function(status) {
	// $('gogoStatus').removeClass('on');
	// $('gogoStatus').removeClass('connect');
	// if (status=='on') {
	// 	$('gogoStatus').addClass('on');
